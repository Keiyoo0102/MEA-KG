...
# Output Files
BEST_MODEL_PATH = os.path.join(MODEL_DIR, 'best_bert_crf_model.pth')
HISTORY_CSV_PATH = os.path.join(RESULT_DIR, 'bert_crf_training_history.csv')

# Entity Types (Must match your annotation schema)
ENTITY_TYPES = [
    'GeologicFeature',
    'LocationRegion',
    'MissionSpacecraft',
...
# Model Settings
# Use standard Hub identifier for portability. 
# You can replace this with a local path if needed.
...
def load_conll_file(file_path):
    ...
    for block in sentence_blocks:
        ...
        for line in block.split('\n'):
            ...
            # Flexible split (Tab or Space)
            parts = line.split('\t')
            if len(parts) < 2:
                parts = line.split(' ')
            
            if len(parts) >= 2:
                token, tag = parts[0], parts[-1] # Take last element as tag
                tokens.append(token)
    ...
    for i, label in enumerate(examples["ner_tags"]):
        ...
        for word_idx in word_ids:
            if word_idx is None:
                ...
                label_ids.append(label[word_idx]) # First sub-token gets label
            else:
                label_ids.append(-100) # Subsequent sub-tokens get ignored
...
try:
    ...
    if len(raw_datasets['train']) == 0:
        raise ValueError("Training set is empty. Check input file format.")
...
class BertCrfForNer(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels=None, token_type_ids=None):
        ...
        if labels is not None:
            # Handle -100 in labels for CRF compatibility
...
# Initialize Model
model = BertCrfForNer(num_labels=len(LABELS_LIST)).to(DEVICE)
        ...
        for i, pred_ids in enumerate(preds):
            ...
            for j, t_id in enumerate(true_ids):
                if t_id != -100:
                    ...
                    # Ensure prediction length matches truth
                    if j < len(pred_ids):
...
def train():
    ...
    plot_training_history(df, "BERT-CRF")
    return df

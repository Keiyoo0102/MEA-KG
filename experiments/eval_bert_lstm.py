...
# Output Files
BEST_MODEL_PATH = os.path.join(MODEL_DIR, 'best_bert_lstm_model.pth')
HISTORY_CSV_PATH = os.path.join(RESULT_DIR, 'bert_lstm_training_history.csv')
...
# --- 3. Model Architecture (BERT + BiLSTM, No CRF) ---

class BertLstmForNer(nn.Module):
    def __init__(self, num_labels, model_name=MODEL_CHECKPOINT, lstm_hidden_size=LSTM_HIDDEN_SIZE, lstm_layers=LSTM_LAYERS):
        ...
        # Classifier (Input is LSTM hidden size * 2)
        self.classifier = nn.Linear(lstm_hidden_size * 2, num_labels)
        
        # Loss function (Standard CrossEntropy, ignoring padding)
    ...
    def forward(self, input_ids, attention_mask, labels=None, token_type_ids=None):
        ...
        if labels is not None:
            # Calculate Loss
            # Flatten to [batch * seq_len, num_labels]
...
# Initialize Model
model = BertLstmForNer(num_labels=len(LABELS_LIST)).to(DEVICE)
...
def evaluate(model_to_eval, dataloader):
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            _, logits = model_to_eval(**batch)
        
        # Get predictions using Argmax (No CRF decoding)
...
def train():
    ...
    plot_training_history(df, "BERT-LSTM")
    return df
